...
**Built:**
- `GET /clients/:id/reports/cart-products` (`api/src/routes/reports.ts`) — groups `cart_events` by `product_id` (the more reliable identifier; the fallback theme-snippet add-to-cart listener only ever sends an id, never a name), picking a representative `product_name` via `MAX()`. Reuses MOF's exact per-visitor "no purchase after this add" abandonment definition, just grouped by product instead of summed across all of them. Returns every product with any view/add/checkout activity in range, sorted by add-to-cart count, no limit/pagination. Added to `ownership.ts`'s `RESOLVERS` table immediately (the exhaustive fail-closed list — a route missing from it 500s on every call, a documented recurring gap in this app, see the 2026-07-30 entry).
- Dashboard: new "Products Added to Cart" table on Funnel's MOF tab (`funnel-client.tsx`, ecommerce niche only, same place Cart Abandonment already lives), full product list with Views/Added to Cart/Checkout Started/Abandonment Rate/Cart Value columns.
- Funnel now supports `?stage=` deep-linking (matching Campaigns' `?view=` and Recommendations' `?type=`), including the same same-route-navigation `useEffect` fix documented in the 2026-08-01 nav-consolidation entry (a link to a different `?stage=` while already on Funnel is a client-side transition, not a remount, so a plain `useState` initializer wouldn't otherwise pick up the change).
- Overview's Cart → Purchase Conversion donut (`overview-client.tsx`) is now a real link straight to `/funnel?stage=mof`, with a "See products added to cart →" caption under it. Wraps only the chart, not the whole card — `CardTitle`'s `InfoTooltip` is its own interactive trigger, and a `<button>` can't nest inside an `<a>`.

**Real gap surfaced, not a bug in this feature:** live-tested against Nothing But Buckets' real data, the new table shows a long tail of "(unnamed product)" rows with $0 cart value, each a distinct `product_id` that never got a resolved name/price. Root cause is the existing, known distinction between the two add-to-cart capture paths (ISSUE_LOG's Step 8 / ecommerce-tracking history): the Shopify Customer Events pixel's `product_added_to_cart` subscriber pulls real title/price from Shopify's cart-line data, but the fallback generic `/cart/add` form-submit listener (for themes/clients without that pixel installed) only ever captures the variant id, no name or value. Not fixed here — this table just makes that pre-existing tracking gap visible per-product for the first time, which is arguably a feature: it tells you exactly which clients still need the richer Customer Events pixel installed.

**Verified:** `tsc --noEmit` and `npm test` (128/128) clean on the API side, `tsc --noEmit` and `next build` clean on the dashboard. Live-tested against real Nothing But Buckets data: clicked the Overview donut, landed on `/funnel?stage=mof` with the MOF tab correctly pre-selected, product table rendered real names (e.g. "North Carolina A&T Bucket Hat", 45 adds, 68.9% abandonment, $1,784 cart value) sorted by add-to-cart count, confirmed the list runs well past the first 20 rows down to single-digit counts with no truncation.
...
**Built:**
- **Campaigns additions** (`campaigns-client.tsx`): `DataDrivenAttributionCard` and `CreativePatternsCard`, ported from the two standalone pages' logic almost verbatim (same queries, same rendering), just restyled as cards and — for the attribution one — taking `from`/`to` as props instead of its own `useDateRangeState` call, so it shares the page's one date range like every other card there. Rendered conditionally: Data-Driven Attribution on the Campaign tab (next to the existing First-Touch/Last-Touch or U-Shaped/Time-Decay table), Creative Patterns on the Creative tab. Both `return null` when there's no data, matching `JourneyPathsCard`'s established "hide, don't show an empty state" convention on this page. The two old standalone client component files (`markov-attribution-client.tsx`, `creative-patterns-client.tsx`) were deleted outright, not left as dead code.
- **Recommendations hub** (`recommendations/recommendations-client.tsx`): one page, a `SegmentedToggle` for the five flagged-item types, `?type=` deep-linkable (same pattern as Campaigns' `?view=`). Each type's existing component (`PauseCandidatesClient` etc.) was kept fully intact internally — same queries, same mutations, same status-filter toggles — only its own page chrome (`ClientKicker`/`h1`/outer `p-6`) was stripped out, since the hub now supplies that once. Zero behavior change to any individual type's own logic.
- **Experiments hub** (`experiments/experiments-client.tsx`): same pattern for Incrementality + Geo-Lift. Both components' own cross-reference text ("Geo-Lift Testing, in the sidebar") was updated to "the Geo-Lift tab" etc., since it's no longer a sidebar item.
- **Old routes** (`/pause-candidates`, `/budget-reallocation`, `/creative-fatigue`, `/tracking-health`, `/invalid-traffic`, `/incrementality`, `/geo-lift`, `/data-driven-attribution`, `/creative-patterns`) now redirect client-side to their new home (`RouteRedirect`, a new shared component mirroring `src/app/page.tsx`'s own root-level client-side redirect — this app's auth token only lives in the browser, so a server-side `redirect()` would fire before it's readable) instead of 404ing for anyone with an old bookmark.
- `nav-items.ts`: Testing & Automation went from 8 items to 3 (Remarketing, Recommendations, Experiments); Reports' Reporting group lost 2 (Creative Patterns, Data-Driven Attribution). `help-content.ts` updated to match — the 5+2 standalone entries collapsed into one "Recommendations" and one "Experiments" entry (each keeping every type's own definition as a `metrics` row), and Campaigns' entry absorbed the two folded-in cards' descriptions.
- `notifications-bell.tsx`: its 4 hardcoded links (Pause Candidates/Creative Fatigue/Tracking Health/Budget Reallocation) now point at `recommendations?type=...` instead of their old standalone routes.

**Real bug caught during live verification, not shipped broken:** the notifications bell linking to a *different* `?type=` while already on the Recommendations page changed the URL but left the visible tab unchanged — clicking "Pause candidates" while viewing Creative Fatigue silently did nothing. Root cause: `useState(initialFromSearchParams)` only reads the URL on first mount; navigating between two `?type=` values on the *same* route is a client-side transition, not a remount, so the initializer never re-runs. This is the same pattern Campaigns' own `?view=` toggle uses and — by inspection — carries the identical latent bug, just never exposed before because every entry point into Campaigns happened to come from a different page (a real remount). Fixed here with a `useEffect` that re-syncs state whenever the URL's own `type` param changes, without touching any manual-toggle-click behavior. Not backported to Campaigns' `?view=` in this pass — flagged here so a future session doesn't have to re-diagnose it if it ever surfaces there.

**Verified:** both workspaces (`tsc --noEmit`, `next build`) clean. Live-tested against real Nothing But Buckets data in the dashboard: Data-Driven Attribution card renders real channel-impact data on the Campaign tab; Creative Patterns card correctly renders nothing for this client (no AI-tagged creatives yet, same empty-state convention as the rest of the page); Recommendations hub's outer type-toggle and each type's own inner status-toggle both work and show real flagged ads; Experiments hub's Incrementality/Geo-Lift toggle works with updated cross-reference copy; old routes (`/creative-fatigue` confirmed directly) redirect correctly; the notifications-bell same-page navigation bug above was caught, fixed, and re-verified fixed; Help & Info's Testing & Automation section now shows exactly 2 entries matching the new nav.
